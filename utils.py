...
class ngsimDataset(Dataset):
    ...
    def __init__(self, mat_file, t_h=30, t_f=10, d_s=2, enc_size=64, grid_size=(13, 3)):
        self.D = mat73.loadmat(mat_file)['traj']
        self.T = mat73.loadmat(mat_file)['tracks']

        self.D = np.array(self.D)
        # self.T = np.array(self.T)
        #
        # self.D = h5py.File(mat_file, 'r')['traj']
    ...
    def getHistory(self, vehId, t, refVehId, dsId):
        if vehId == 0:
            ...
            # if len(self.T[0]).shape[1]<=vehId-1:
            if len(self.T[0]) <= vehId - 1:
                return np.empty([0, 2])
            refTrack = self.T[dsId - 1][refVehId - 1].transpose()  # take the target vehicle as the reference point
            # print(dsId)
            # print(vehId)
            # print('---')
            # print(self.T[dsId - 1][vehId - 1])
            if self.T[dsId - 1][vehId - 1] is None:
                return np.empty([0, 2])
    ...
    def collate_fn(self, samples):
        ...
        if nbr_batch_size != 0:
            ...
            pos = [0, 0]
            mask_batch = torch.zeros(len(samples), self.grid_size[1], self.grid_size[0],
                                     self.enc_size)  # gird_size (13, 3) width 3, height 13, depth lenth of output of lstm cells
            ...
            op_mask_batch = torch.zeros(self.t_f // self.d_s, len(samples),
                                        2)  # future timestep, number of vehs, two locations x and y
            ...
            for sampleId, (hist, fut, nbrs, lat_enc, lon_enc, vehId, t, ds) in enumerate(samples):
                ...
                hist_batch[0:len(hist), sampleId, 0] = torch.from_numpy(
                    hist[:, 0])  # NOTE here it is [0:len(hist), ...] not maxlen
